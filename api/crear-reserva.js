...
export default async function handler(req, res) {
  ...
  const { barco_id, fecha, turno, nombre, email, telefono } = req.body;

  if (!barco_id || !fecha || !turno || !nombre || !email) {
  ...
  try {
    ...
    // Crear reserva en BD (se confirmará definitivamente via webhook)
    ...
    const form = createRedirectForm({
      ...
      DS_MERCHANT_AMOUNT: String(precioData.precio * 100), // en céntimos
